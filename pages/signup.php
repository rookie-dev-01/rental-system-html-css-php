...
else
{
 ...
 if(filter_var($emailid, FILTER_VALIDATE_EMAIL) === false) 
  ...
  else
  {
   ...
   {
  $qry= "INSERT INTO user VALUES('','$emailid','','','','','$pwd')";
          ...
          <section class="4u" >
            <div class="box" >
                <form action='#' method='post' >
                    <h2>Email id:</h2></td><td><input type='text' name='emailid'/>
                    <h2>Password:</h2></td><td><input type='password' name='password'/>
                    <h2>Confirm Password:</h2></td><td><input type='password' name='re-password'/><br/><br/>
...
{
 if($pwd!=$repwd)
  echo "<h2>Re-enter the password</h2>";
 else
 {
 if(filter_var($emailid, FILTER_VALIDATE_EMAIL) === false) 
  {
    echo("<h2>Entered emailid is not a valid emailid</h2>");
  } 
  else
  {
   $query=mysql_query("select * from user where emailid='".$emailid."' and password='".$pwd."'") or die(mysql_error());
   ...
   if($res)
   {
    echo "<h2>This emailid is already registered please Login</h2>";
   }
